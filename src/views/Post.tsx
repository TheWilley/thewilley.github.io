...
function Post() {
    ...
    useEffect(() => {
        ...
        fetch(`http://localhost:1337/api/blog-posts/${id}?populate=*`).then(data => data.text()).then(posts => setPost(JSON.parse(posts)))
    }, [])

    return (
        <motion.div initial={{ transform: 'scale(0.8)', opacity: 0 }} animate={{ transform: 'scale(1)', opacity: 1 }} exit={{ transform: 'scale(0.8)', opacity: 0 }}>
            <img src={`http://localhost:1337${post?.data.attributes.thumbnail.data.attributes.url}`} className='h-96 max-h-full w-full object-cover' />
            <div>
